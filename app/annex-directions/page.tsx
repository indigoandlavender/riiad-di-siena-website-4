...
export default function AnnexDirectionsPage() {
  ...
  const getDisplayDirections = (): Direction[] => {
    ...
    const annexSteps = directions.annex.map((d, idx) => ({
      ...d,
      Step_Number: String(sharedSteps.length + idx + 1)
    }));
  ...
  const handleDownloadPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html dir="${isRTL ? 'rtl' : 'ltr'}">
        <head>
          <title>Walking Directions - The Douaria (No. 35)</title>
          <style>
        ...
        <body>
          ...
          <p class="subtitle">To The Douaria (No. 35)</p>
          <div class="note">
    ...
    <div className="min-h-screen pt-24 bg-sand">
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-8">
          ...
          {(() => {
            const subtitle = getText("annex_subtitle", "To The Douaria (No. 35)");
            return (
        ...
        {/* Link to Main Riad Directions */}
        <div className="flex justify-center mb-8">
          {(() => {
            const linkText = getText("link_to_riad", "← Looking for Riad di Siena (No. 37)?");
            return (
              <Link 
                href="/riad-directions"
